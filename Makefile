IMAGE_NAME = tiagogarciaferreira/app-name
TAG ?= latest
...
add-host:
	@printf "\n"
	@echo "🌐 Adding host entry for api.app.name.tgfcodes.com..."
	@HOST_ENTRY="127.0.0.1       api.app.name.tgfcodes.com"; \
	if ! grep -q "api.app.name.tgfcodes.com" /etc/hosts; then \
...
compose-clean:
	...
	@docker stop grafana prometheus app postgres 2>/dev/null || true
	@docker rm -f grafana prometheus app postgres 2>/dev/null || true
	@docker volume rm postgres_data grafana_data prometheus_data 2>/dev/null || true
	@echo "✨ Cleanup completed successfully!"

compose-up:
	@printf "\n"
	@echo "🚀 Starting containers..."
	@docker compose -p "app-name" -f docker-compose.yaml --env-file ./.env up -d
...
run-tests-k6:
	...
	@eval "$$(grep -E '^(HOSTNAME|SERVER_PORT|API_VERSION|USER_K6|USER_K6_PASSWORD)=' .env | sed 's/^/export /')" && \
	k6 run --insecure-skip-tls-verify k6/scenarios/read_app-name.js
...
help:
	...
	@echo "🧪 Tests"
	@echo "--------------------------------------------------"
	@echo "  run-tests-k6         Run k6 tests for read_app-name scenario"
	@echo ""
